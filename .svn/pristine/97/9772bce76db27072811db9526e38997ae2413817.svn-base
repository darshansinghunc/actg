...
class project:
    ...
    def _flowveclists2fdmpval(self,infdm,flowveclist1,flowveclist2,numpart,numperm,minparts=8):
        #print 'xln1820',flowveclist1,flowveclist2
        if numpart==1:
            pvalue=-1.0
            return (infdm,pvalue)
        else:
            flowveclist1copy=copy.deepcopy(flowveclist1)
            flowveclist2copy=copy.deepcopy(flowveclist2)
            maxdelete=len(flowveclist1copy)-minparts
            numdeleted=0
            for flow in flowveclist1copy:
                if sum(flow)<0.96 and (numdeleted<maxdelete):
                    flowveclist1copy.remove(flow)
                    numdeleted+=1
            numdeleted=0
            for flow in flowveclist2copy:
                if sum(flow)<0.96 and (numdeleted<maxdelete):
                    flowveclist2copy.remove(flow)
                    numdeleted+=1                    
            fdmnulllist=[]
            for i in range(numperm):
    ...
    def mergeffullfiles(self):
        ...
        if not os.path.isfile(ffullallfile):
            ...
            for f in [x for x in os.listdir(ffulldir) if x!='%s_ALL_ffull.txt'%self.cfg.project_name]:
                #print f
                if os.path.isfile('%s/%s'%(ffulldir,f)):
